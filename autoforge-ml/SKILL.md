---
name: autoforge-ml
description: Autonomous optimization loop for machine learning models (scikit-learn, XGBoost, LightGBM, CatBoost, random forests, SVMs, neural nets, etc.). Iteratively improves model performance through feature engineering, hyperparameter tuning, model selection, and ensemble methods. Use when asked to "optimize model", "tune model", "automl", "improve prediction", or "ML optimization".
...
# AutoForge-ML — Autonomous Machine Learning Optimization

You are an autonomous ML researcher. You iteratively improve model performance by modifying training code, running experiments, and keeping only what works.

This skill is designed for machine learning models: scikit-learn, XGBoost, LightGBM, CatBoost, random forests, gradient boosting, SVMs, KNN, ExtraTrees, neural networks (PyTorch, TensorFlow), and similar. For pure statistical models (linear regression, ARIMA, GLMs), use `/autoforge-stats` instead.
...
```bash
git checkout -b autoforge-ml/<tag>
```
...
10. **Try different algorithms** for the problem type:
    - Classification: RandomForest, GradientBoosting, XGBoost, LightGBM, CatBoost, SVM, KNN, ExtraTrees, MLPClassifier
    - Regression: RandomForest, GradientBoosting, XGBoost, LightGBM, CatBoost, SVR, KNN, MLPRegressor
    - Time series (ML approach): LightGBM/XGBoost with lag features, rolling statistics, Fourier features
11. **Try simpler models**: Sometimes a single well-tuned model beats a complex ensemble.
...
13. **SVM**: C, gamma, kernel
14. **Neural networks**: hidden_layer_sizes, learning_rate_init, alpha, batch_size, activation
...
### Classification
- Start with: LightGBM or XGBoost (depending on data size)
...
### Regression
- Start with: LightGBM or GradientBoosting
...
### Time Series (ML approach)
- Start with: LightGBM with lag features and rolling statistics
- Key metrics: RMSE, MAPE, directional accuracy
- Watch for: temporal leakage (never use future data), seasonality, trend
- Feature ideas: lag features, rolling mean/std/min/max, Fourier features for seasonality, calendar features

### Survival Analysis
- Start with: Random Survival Forest, GradientBoostingSurvival
...
```
commit	metric_value	cv_std	memory_mb	train_time_s	status	description
a1b2c3d	0.8234	0.0045	128.0	5.2	keep	baseline: random forest
...
i9j0k1l	0.8401	0.0036	520.0	22.1	keep	stack LightGBM + XGBoost + Ridge meta-learner
```
